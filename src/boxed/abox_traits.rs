...
impl<T: ?Sized, A: Free> Deref          for ABox<T, A> { fn deref(&self)             -> &T       { unsafe { self.data().as_ref() } } type Target = T; }
impl<T: ?Sized, A: Free> DerefMut       for ABox<T, A> { fn deref_mut(&mut self)     -> &mut T   { unsafe { self.data().as_mut() } } }
